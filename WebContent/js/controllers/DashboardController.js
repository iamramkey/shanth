...
SignatureApp.controller('DashboardController', function($rootScope, $scope, $http,$timeout) {
	...
	$(".view_more_trend").click(function(){
	...
	$(document).on("change","#dealIdList",function(){
		var accNameList = $('#accNameList').val(),
			dealIdList = $('#dealIdList').val();
		toolbarItemClickFunction('dealdashboard','Deal Dashboard',' icon-bar-chart ');
		window.location.href = '#/dealdashboard?accNameList='+accNameList+'_&_dealIdList='+dealIdList;
	});
	...
	$(document).on("click", "li.deal_dashboard" , function() {
		...
		var deal = $(this).attr('data-deal');
	  	toolbarItemClickFunction('dealdashboard','Deal Dashboard',' icon-bar-chart ');
		window.location.href = '#/dealdashboard?accNameList='+account+'_&_dealIdList='+deal;
	});
	...
	$(document).on('click', '#dealDashboar-body-1 tr', function(){
		if(!$(this).attr('role')) return;
		var newTableValue=  $('#dealDashboa-table2').dataTable();
    	var row=newTableValue.api().row(this).data();
		window.location.href = '#/dealdashboard?accNameList='+row.carrier+'_&_dealIdList='+row.deal;
	});
	$(document).on('click', '#dealDashboar-body tr', function(){
		if(!$(this).attr('role')) return;
		var newTableValue=  $('#dealDashboa-table1').dataTable();
    	var row=newTableValue.api().row(this).data();
		window.location.href = '#/dealdashboard?accNameList='+row.carrier+'_&_dealIdList='+row.deal;
	});
	...
	function fillchangetrafficlist(result){
		...
		for(var i = 0; i < result.changesInTrafficDataList.length; i++){
			...
			if(result.changesInTrafficDataList[i].direction == 'IN')
	        {
		      ...
		      inSeriesTraffic += '<div class="col1 db_toolTip" data-toggle="popover" data-placement="bottom" data-trigger="hover" data-content="'+result.changesInTrafficDataList[i].data+'" ><div class="cont"><div class="cont-col1">';
		...
		for(var i = 0; i < result.changesInTrafficDataList.length; i++){
	        if(result.changesInTrafficDataList[i].direction == 'OUT') {
		      ...
		      outSeriesTraffic += '<div class="col1 db_toolTip" data-toggle="popover" data-placement="bottom" data-trigger="hover" data-content="'+result.changesInTrafficDataList[i].data+'" ><div class="cont"><div class="cont-col1">';
		...
		for(var i = 0; i < result.changesInTrafficDataList.length; i++){
	      ...
	      allSeriesTraffic += '<div class="col1 db_toolTip" data-toggle="popover" data-placement="bottom" data-trigger="hover" data-content="'+result.changesInTrafficDataList[i].data+'"><div class="cont"><div class="cont-col1">';
		...
		var inSeriesContent = '<ul class="feeds" id="inSeriesContent">';
		 for(var i = 0; i < totalRecentActivityArray.length; i++){
			if(primaryFilter_RecentActivity == totalRecentActivityArray[i].group || primaryFilter_RecentActivity=="ALL"){
				...
				inSeriesContent += '<div class="col1 db_toolTip" data-toggle="popover" data-placement="bottom" data-trigger="hover" data-content="'+totalRecentActivityArray[i].data+'"><div class="cont"><div class="cont-col1">';
	...
	function filldealexpiringlist(result){
	    ...
	    for(var i = 0; i < result.dealExpiringDataList.length; i++){
			...
			inSeriesContent += '<div class="col1 db_toolTip" data-toggle="popover" data-placement="bottom" data-trigger="hover" data-content="'+result.dealExpiringDataList[i].data+'"><div class="cont"><div class="cont-col1">';
		...
		for(var i = 0; i < totalfillshortfalistArray.length; i++){
			if(secondaryFilter_fillshortfalist == totalfillshortfalistArray[i].secondaryFilter){
				if(primaryFilter_fillshortfalist == totalfillshortfalistArray[i].primaryFilter || primaryFilter_fillshortfalist == 'Both'){
					...
					inSeriesShortPer += '<div class="col1 db_toolTip" data-toggle="popover" data-placement="bottom" data-trigger="hover" data-content="'+totalfillshortfalistArray[i].data+'"><div class="cont"><div class="cont-col1">';
	    ...
	    for(var i = 0; i < totalFilltopdestinationslistArray.length; i++){
	    	if(secondaryFilter == totalFilltopdestinationslistArray[i].secondaryFilter){
	    		if(primaryFilter == totalFilltopdestinationslistArray[i].primaryFilter || primaryFilter == 'Both'){
	    			TopDestination_all += '<li class="country_dashboard" data-country="'+totalFilltopdestinationslistArray[i].country+'">';
					TopDestination_all += '<div class="col1 db_toolTip" data-toggle="popover" data-placement="bottom" data-trigger="hover" data-content="'+totalFilltopdestinationslistArray[i].data+'"><div class="cont"><div class="cont-col1">';
	...
	function fillTrendMarginDetails(result){
		...
		for(var x=0; x < result.headers.length; x++) {
			HeaderingRow+= '<th>'+result.headers[x]+'</th>';
		}
		$('#dealDashboar-thead-margin').append(HeaderingRow);
...
// $(".db_toolTip").mouseover(function(){
$(document).on("mouseover",".db_toolTip",function(){
