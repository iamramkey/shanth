...
import se.signa.signature.dio.BDBVolumeRevenueResult;
...
@Path("/bdbvolumerevenue")
public class BDBVolumeRevenueApi extends StigWebApi
{
	private static Logger logger = Logger.getLogger(BDBVolumeRevenueApi.class);
	...
	public StigWebResult bdbVolumeRevenue(BusinessDashboardInput input)
	{
		input.validate();
		input.checkRules();
		...
		getDailyVolumes(dailyVolumesList, "In");
		getDailyVolumes(dailyVolumesList, "Out");

		List<DailyRevenue> dailyRevenueList = new LinkedList<DailyRevenue>();

		getDailyRevenue(dailyRevenueList, "In");
		getDailyRevenue(dailyRevenueList, "Out");

		String notification = "Volume and revenue details Rendered Successfully !!!";
		logger.info(notification);
		return new BDBVolumeRevenueResult(notification, headers, dailyVolumesList, dailyRevenueList);
	}

	private void getDailyVolumes(List<DailyVolumes> dailyVolumesList, String direction)
	{
		DealTierLite dlt = getDealTiers(direction, "VOL");
		TrafficCount trc = getTrafficCount(direction, "VOL");
	...
	private void getDailyRevenue(List<DailyRevenue> dailyRevenueList, String direction)
	{
		DealTierLite dlt = getDealTiers(direction, "REV");
		TrafficCount trc = getTrafficCount(direction, "REV");
	...
	private TrafficCount getTrafficCount(String direction, String filter)
	{
		String query = "";
		...
		try
		{
			...
			if (result.next())
			{
				...
				while (oneWeekBefore.isBefore(now))
				{
					dayUsage[i] = getTrafficCountOnDate(direction, now.withTimeAtStartOfDay(), now.withTimeAtStartOfDay().plusDays(1).minusSeconds(1), filter);
					now = now.minusDays(1);
					i++;
				}
	...
	private String getRevenueTrafficCountQuery(String direction)
	{
		...
		{
			query = "select round(sum(IN_DEALAMT)) as rev_billed_usage from margin_summary where in_deal_id != 0 and in_deal_id is not null and evt_dttm <= sysdate " + " and in_deal_id in ( select del_id from " + refName + ".deal where del_to_dttm >=sysdate ) ";
		}
		else if (direction.contains("O"))
		{
			query = "select round(sum(OUT_DEALAMT)) as rev_billed_usage from margin_summary where out_deal_id != 0 and out_deal_id is not null and evt_dttm <= sysdate " + " and out_deal_id in ( select del_id from " + refName + ".deal where del_to_dttm >=sysdate ) ";
		}
	...
	private String getVolumeTrafficCountQuery(String direction)
	{
		...
		String query = "";
		if (direction.contains("I"))
			query = "select round(sum(in_rev_billed_usage)/60.00) as rev_billed_usage from margin_summary where in_deal_id != 0 and in_deal_id is not null and evt_dttm <= sysdate" + " and in_deal_id in ( select del_id from " + refName + ".deal where del_to_dttm >=sysdate ) ";
		else if (direction.contains("O"))
			query = "select round(sum(out_rev_billed_usage)/60.00) as rev_billed_usage from margin_summary where out_deal_id != 0 and out_deal_id is not null and evt_dttm <= sysdate " + " and out_deal_id in ( select del_id from " + refName + ".deal where del_to_dttm >=sysdate ) ";
	...
	private Double getTrafficCountOnDate(String direction, DateTime fromDttm, DateTime toDttm, String filter)
	{
		String query = "";
		if (filter.equalsIgnoreCase("VOL"))
	...
	private String getTrafficCountOnDateQuery(String direction)
	{
		...
		if (direction.contains("I"))
			query = "select round(sum(in_rev_billed_usage)/60.00) as rev_billed_usage from margin_summary where in_deal_id != 0 and in_deal_id is not null and evt_dttm >= ? and evt_dttm <= ? ";
		else if (direction.contains("O"))
			query = "select round(sum(out_rev_billed_usage)/60.00) as rev_billed_usage from margin_summary where out_deal_id != 0 and out_deal_id is not null and evt_dttm >= ? and evt_dttm <= ? ";
	...
	private String getRevenueTrafficCountOnDateQuery(String direction)
	{
		String query = "";
		if (direction.contains("I"))
			query = "select round(sum(IN_DEALAMT)) as rev_billed_usage from margin_summary where in_deal_id != 0 and in_deal_id is not null and evt_dttm >= ? and evt_dttm <= ? ";
		else if (direction.contains("O"))
			query = "select round(sum(OUT_DEALAMT)) as rev_billed_usage from margin_summary where out_deal_id != 0 and out_deal_id is not null and evt_dttm >= ? and evt_dttm <= ? ";
	...
	private DealTierLite getDealTiers(String direction, String filter)
	{

		String query = "";
		if (filter.equalsIgnoreCase("VOL"))
			query = "select dlt_direction , sum(dlt_to_val) as dlt_to_val  from deal_tier where dlt_direction like ? and dlt_comm_fl = 'Y'  and del_id in ( select del_id from deal where del_to_dttm >=sysdate ) group by dlt_direction";
		else
		{
			...
			query = query + " where dlt_direction like ? and dlt_comm_fl='Y' and dlt.del_id in ( select del_id from deal where del_to_dttm >=sysdate ) group by dlt_direction";
		}
		...
		try
		{
			result = RefDBHelper.getDB().execute(query, new Object[] { "%" + direction + "%" });
			if (result.next())
			{
