...
@Path("/userdeactivate")
public class UserDeActivateApi
{
	...
	public StigWebResult userDeActivate(UserModifyInput input)
	{
		input.validate();
		input.checkRules();
		UserTbl userTbl = UserTblDba.getI().fetchByPk(input.id);
		userTbl.setUsrStatus(Constants.USER_DEACTIVED_STATUS);
		UserTblDba.getI().update(userTbl, input.usrId);
		String notification = "User Status DeAtivated  for User Name : " + userTbl.getUsrName();
