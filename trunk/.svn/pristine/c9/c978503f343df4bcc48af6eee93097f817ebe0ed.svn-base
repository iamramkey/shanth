...
import se.signa.signature.dio.CDBDealsCountResult;
...
@Path("/cdbdealscount")
public class CDBDealsCountApi extends StigWebApi
{
	private static Logger logger = Logger.getLogger(CDBDealsCountApi.class);
	...
	public StigWebResult cdbDealsCount(CountryDashboardInput input)
	{
		input.validate();
		input.checkRules();
		...
		getNewDealsData(newDealsList, input.countryName);

		List<ActiveDeals> activeDealsList = new LinkedList<ActiveDeals>();
		getActiveDealsData(activeDealsList, input.countryName);

		String notification = "Deals Count Rendered Successfully !!!";
		logger.info(notification);
		return new CDBDealsCountResult(notification, newDealsList, activeDealsList);
	}

	private void getActiveDealsData(List<ActiveDeals> newDealsList, String country)
	{
		...
		try
		{
			...
			query.append("  in_dbg_id from  margin_summary where evt_dttm <= sysdate and in_bnd_id in (select bnd_id from ");
			query.append(" " + refName + ".area_connection arc, " + refName + ".country con, " + refName + ".area ar where con.CTR_ISO_CD=ar.CTR_ISO_CD and ");
			query.append(" arc.ect_to_elt_id=ar.elt_id and con.ctr_name = ?) group by in_deal_id , in_dbg_id ) m1  on m1.in_dbg_id = dlt1.dbg_id  ");
			...
			query.append(" out_dbg_id from  margin_summary where evt_dttm <= sysdate and out_bnd_id in (select bnd_id from ");
			query.append(" " + refName + ".area_connection arc, " + refName + ".country con, " + refName + ".area ar where con.CTR_ISO_CD=ar.CTR_ISO_CD and ");
			query.append(" arc.ect_to_elt_id=ar.elt_id and con.ctr_name = ?) group by out_deal_id  , out_dbg_id ) m2 on m2.out_dbg_id = dlt2.dbg_id  ");
			...
			result = UsageDBHelper.getDB().execute(query.toString(), new Object[] { country, country });
			while (result.next())
			{
	...
	private void getNewDealsData(List<NewDeals> newDealsList, String country)
	{
		String refName = RefDBHelper.getDB().getUserName();
		ExecuteSelectStamentResult result = null;
		try
		{
			...
			query.append(" in_dbg_id from  margin_summary where evt_dttm <= sysdate and in_bnd_id in (select bnd_id from ");
			query.append(" " + refName + ".area_connection arc, " + refName + ".country con, " + refName + ".area ar where con.CTR_ISO_CD=ar.CTR_ISO_CD and ");
			query.append(" arc.ect_to_elt_id=ar.elt_id and con.ctr_name = ?)  group by in_deal_id, in_dlt_id , in_dbg_id ) m1  on m1.in_dbg_id = dlt1.dbg_id ");
			query.append(" where dlt1.del_id=del1.del_id   and dlt1.dlt_comm_fl = 'Y' ");
			query.append(" and dlt1.dlt_direction = 'In'  and  dlt1.del_id in ");
			...
			query.append(" out_dbg_id from  margin_summary where evt_dttm <= sysdate and out_bnd_id in (select bnd_id from ");
			query.append(" " + refName + ".area_connection arc, " + refName + ".country con, " + refName + ".area ar where con.CTR_ISO_CD=ar.CTR_ISO_CD and ");
			query.append(" arc.ect_to_elt_id=ar.elt_id and con.ctr_name = ?) group by out_deal_id , out_dlt_id , out_dbg_id ) m2 on m2.out_dbg_id = dlt2.dbg_id ");
			query.append(" where dlt2.del_id=del2.del_id  and dlt2.dlt_comm_fl = 'Y' and dlt2.dlt_direction = 'Out' and  ");
