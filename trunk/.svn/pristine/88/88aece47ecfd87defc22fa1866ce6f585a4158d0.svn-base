...
import se.signa.signature.dio.CDBShortfallWarningListResult;
...
@Path("/cdbshortfallwarninglist")
public class CDBShortfallWarningListApi extends StigWebApi
{
	private static Logger logger = Logger.getLogger(CDBShortfallWarningListApi.class);
	...
	public StigWebResult cdbShortfallWarningList(CountryDashboardInput input)
	{
		input.validate();
		input.checkRules();
		...
		getShortFallWarning(shortfallWarningDataList, "I", "PER", input.countryName);
		getShortFallWarning(shortfallWarningDataList, "I", "VOL", input.countryName);
		getShortFallWarning(shortfallWarningDataList, "O", "PER", input.countryName);
		getShortFallWarning(shortfallWarningDataList, "O", "VOL", input.countryName);

		String notification = "Short fall warning List Rendered Successfully !!!";
		logger.info(notification);
		return new CDBShortfallWarningListResult(notification, shortfallWarningDataList);
	}

	private void getShortFallWarning(List<ShortfallWarningData> shortfallWarningDataList, String direction, String filter, String country)
	{
	...
	private String getShortFallWarningQuery(String direction)
	{
		...
		if (direction.contains("I"))
		{
			...
			query = query + " del.acc_id = acc.acc_id and msu.evt_dttm >= (sysdate -8) and msu.in_bnd_id in (select bnd_id from ";
			query = query + " " + refName + ".area_connection arc, " + refName + ".country con, " + refName + ".area ar where con.CTR_ISO_CD=ar.CTR_ISO_CD and ";
			query = query + " arc.ect_to_elt_id=ar.elt_id and con.ctr_name = ?) and msu.evt_dttm < (sysdate-1)  group by dbg.dbg_name,del.del_name,del.del_id,acc.acc_company_name, dbg.dbg_id ";
			query = query + " ) m1, ";
			query = query + " (select sum(in_rev_billed_usage) as rev_billed_usage,dbg.dbg_name,del.del_id ";
			query = query + " , acc.acc_company_name, dbg.dbg_id from margin_summary msu , " + refName + ".deal_band_group dbg , " + refName + ".deal del , ";
			query = query + " " + refName + ".customer_vendor acc where msu.in_dbg_id = dbg.dbg_id and msu.in_deal_id = del.del_id and  ";
			query = query + " del.acc_id = acc.acc_id and msu.in_bnd_id in (select bnd_id from ";
			query = query + " " + refName + ".area_connection arc, " + refName + ".country con, " + refName + ".area ar where con.CTR_ISO_CD=ar.CTR_ISO_CD and ";
			query = query + " arc.ect_to_elt_id=ar.elt_id and con.ctr_name = ?) and msu.evt_dttm < (sysdate -1) group by dbg.dbg_name,del.del_id ,acc.acc_company_name, dbg.dbg_id  order by del_id desc ";
		...
		else if (direction.contains("O"))
		{
			...
			query = query + " del.acc_id = acc.acc_id and msu.out_bnd_id in (select bnd_id from ";
			query = query + " " + refName + ".area_connection arc, " + refName + ".country con, " + refName + ".area ar where con.CTR_ISO_CD=ar.CTR_ISO_CD and ";
			query = query + " arc.ect_to_elt_id=ar.elt_id and con.ctr_name = ?) and msu.evt_dttm >= (sysdate -8) and msu.evt_dttm < (sysdate-1)  group by dbg.dbg_name,del.del_name,del.del_id,acc.acc_company_name, dbg.dbg_id ";
			query = query + " ) m1, ";
